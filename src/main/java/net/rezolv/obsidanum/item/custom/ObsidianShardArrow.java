...
public class ObsidianShardArrow extends Item {
    public ObsidianShardArrow(Properties pProperties) {
        super(pProperties);
    }
    public void appendHoverText(ItemStack itemstack, Level world, List<Component> list, TooltipFlag flag) {
        super.appendHoverText(itemstack, world, list, flag);
            list.add(Component.translatable("item.obsidan.description.obsidian_shard_arrow").withStyle(ChatFormatting.GRAY));
    }
    @Override
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        ...
        if (itemInHand.getItem() == ItemsObs.OBSIDIAN_SHARD_ARROW.get()) {
            // Проверка, если не в мире
            if (!pLevel.isClientSide) {
                MobEffectInstance existingEffect = pPlayer.getEffect(EffectsObs.PROTECTION_ARROW.get());
